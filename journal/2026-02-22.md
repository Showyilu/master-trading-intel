...
### 做了什么
- 升级 `scripts/build_authenticated_constraints.py`：在原有 inventory overlay 基础上，新增 Binance 借币维度鉴权覆盖。
  - 调用签名接口 `/sapi/v1/margin/maxBorrowable` 回填 `max_borrow_usd`。
  - 调用签名接口 `/sapi/v1/margin/next-hourly-interest-rate` 回填 `borrow_rate_bps_per_hour`。
  - 与 inventory 一起重算保守仓位上限：`max_position_usd = min(existing_cap, inventory + max_borrow)`。
- 保留 fail-soft 机制：无密钥或接口失败时不打断流水线，继续使用模板约束。
- 更新文档与状态文件：`README.md`、`PLAN.md`、`MEMORY.md`。

### 结果
- 本地验证通过：`python3 scripts/build_authenticated_constraints.py --constraints data/execution_constraints.latest.json --quotes data/normalized_quotes_cex_latest.json` 可正常执行。
- 当前环境无鉴权信息，输出为 `Authenticated rules updated: 0`（符合预期），但借币覆盖逻辑已可运行、待密钥注入后自动生效。
- 交付物已落库：脚本能力从“仅库存”扩展到“库存 + 借币额度 + 借币利率”三要素。

### 阻塞点
- 运行环境仍未注入 Binance/Bybit API key/secret，无法验证实盘借币覆盖率与实际参数值。
- Bybit 借币额度/利率的鉴权接入尚未完成，当前借币实值覆盖仅 Binance 侧可用。

### 下一步
- 注入 Binance/Bybit 密钥，验证约束文件中 `max_borrow_usd` 与 `borrow_rate_bps_per_hour` 的实值替换覆盖率。
- 扩展 Bybit 借币与杠杆层级接口，把 borrow/capacity 约束从单边（Binance）升级为双边一致。
